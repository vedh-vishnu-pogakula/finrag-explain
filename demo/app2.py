"""
AuditRAG -- app 2: the clean single-page demo (two tabs).

App 1 (`streamlit_app.py`) is the multipage deep-dive; this file is the concise one for a
quick walk-through. Both read the same checkpoints and result files.
...
st.set_page_config(page_title="AuditRAG", page_icon="🧾", layout="wide")
...
def main():
    st.title("AuditRAG")
    st.caption("Explainable financial RAG with retrieval attribution and verifier-validated "
               "faithfulness · B.E. major project, CBIT Hyderabad · paper: *Auditing the Auditor*")
    ...
    with st.sidebar:
        ...
        st.caption("Deeper version with the audit playground and failure browser: "
                   "`streamlit run demo/streamlit_app.py`")

    tab_explore, tab_overview = st.tabs(["Explore a question", "Results overview"])
    with tab_overview:
        render_overview()
    with tab_explore:
...
def render_question(dataset: str, qa_id: str, method: str, art: dict):
    rec = art["b1"][qa_id]
    gen = rec["generated"]
